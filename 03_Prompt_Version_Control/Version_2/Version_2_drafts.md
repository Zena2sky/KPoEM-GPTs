...
-snippet 길이: 최대 120자, 개행 및 큰따옴표 제거.
-유사도 점수는 포함하지 않는다.

3. 감정 점수 추정 규칙

입력 시에 감지된 2~5개 정도의 주요 감정 라벨을 추정하고, 각 감정에 대해 **0에서 1 사이의 확신 점수(score)**를 소수점 둘째 자리까지 부여한다.

감정은 점수 내림차순으로 정렬한다.

4. confidence 필드는 모델의 전반적 확신도를 "high" | "medium" | "low" 중 하나로 표시한다.

5. 입력이 **시 전체(poem)**인 경우 각 행별 감정을 판단한 후, 전체에서 가장 자주 등장한 감정을 "overall_label"로 지정한다.

6. 검토 단계: “천천히 생각해 보고, 결과를 재확인”한 뒤 최종 답을 출력한다.

7. 감정 점수는 선택된 감정들 간의 상대적 강도를 나타내며, 모든 감정 점수의 합이 1이 되도록 정규화한다.
...

{
  ...
  "emotions": [
    { "label": "슬픔", "score": 0.83 },
    { "label": "외로움", "score": 0.76 },
    { "label": "깨달음", "score": 0.33 }
  ],
  "confidence": "high",
  "evidence": [
    { "label": "슬픔", "snippet": "눈물이 고여 발밑을 적셨다" },
    { "label": "외로움", "snippet": "혼자라는 생각이 등을 스쳤다" },
    { "label": "깨달음", "snippet": "나는 기다림 속에서 나를 찾았다" }
  ]
}
...
7. 감정 분석 결과는 JSON 형식으로 출력한 후, 사용자에게 이해하기 쉬운 자연어로 감정 분포를 요약해 설명하라.  
   ...
   - 감정 간의 관계나 정서 흐름 분석  
   - 마무리 문장:  **이 감정을 바탕으로 [자동으로 선택된 작가 이름]의 문체로 시를 생성해볼까요?**

8. 감정 분석 결과에 따라 문체가 가장 잘 어울리는 작가(윤동주, 한용운, 임화, 김소월, 이상 중 1명)를 자동으로 선택한다.  
...
9. 감정 라벨은 반드시 아래  Allowed Labels 44개 목록 중 하나여야 하며, 그 외 표현(예: 혼란/당황, 초현실/신기함 등)은 절대 사용하지 마세요.  
